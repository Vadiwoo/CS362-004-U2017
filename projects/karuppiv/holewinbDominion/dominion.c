...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	case smithy:
		return smithyCall(state, handPos);
		/*
		//+3 Cards
		for (i = 0; i < 3; i++)
		{
		...
		*/
	case village:
		return villageCall(state, handPos);
		/*
		//+1 Card
		drawCard(currentPlayer, state);
	...
	case great_hall:
		return greatHallCall(state, handPos);
		/*
		//+1 Card
		drawCard(currentPlayer, state);
...
//beginning of new refactored functions
int gardensCall()
{
	return -1;
}
//this function is for the refactored adventurer card
int adventurerCall(struct gameState *state)
{
	int currentPlayer = whoseTurn(state);
	int nextPlayer = currentPlayer + 1;
...
int villageCall(struct gameState *state, int handPos)
{
	int currentPlayer = whoseTurn(state);
	int nextPlayer = currentPlayer + 1;
	//+1 Card --after bug added it's actually 2
	drawCard(currentPlayer, state);
	//drawCard(currentPlayer, state);
	//+2 Actions
	state->numActions = state->numActions + 2;
...
//refactored great hall card
int greatHallCall(struct gameState *state, int handPos)
{
	int currentPlayer = whoseTurn(state);
	//+1 Card
	drawCard(currentPlayer, state);
