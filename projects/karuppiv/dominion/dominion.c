...
int Adventurer(int currentPlayer, int temphand[MAX_HAND], int z, struct gameState *state) {
	int drawntreasure = 0;
...
int Remodel(int currentPlayer, int choice1, int choice2, struct gameState *state, int handPos) {
	...
	for (i = 0; i < state->hand[currentPlayer]; i++)
	{
...
int Salvager(int currentPlayer, struct gameState* state, int choice1, int handPos)
{
	//+1 buy
	state->numBuys++;
...
int Village(int currentPlayer, struct gameState *state, int handPos)
{
	...
	drawCard(currentPlayer, state);
	//+2 Actions
	state->numActions = state->numActions + 12;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	{
	case adventurer:
		Adventurer(currentPlayer, temphand, z, state);
	...
	case remodel:
		Remodel(currentPlayer, choice1, choice2, state, handPos);
		return 0;
	...
	case salvager:
		Salvager(currentPlayer, state, choice1, handPos);
		return 0;
